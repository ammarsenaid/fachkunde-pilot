...
export default function Glossary() {
  ...
  const categories = useMemo(() => ["all", ...Array.from(new Set(glossary.map((g) => g.category)))], []);
      ...
      <ul className="mt-6 grid gap-3 sm:grid-cols-2">
        {filtered.map((g) => (
          <li key={g.id} className="card-base p-5">
            ...
            {showAr && g.arabic && (
              <p className="mt-3 rounded-xl bg-secondary/60 p-3 font-arabic text-right text-sm leading-loose" dir="rtl">
                {g.arabic}
              </p>
            )}
          </li>
